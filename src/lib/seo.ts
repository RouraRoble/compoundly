...
export function webAppLd(
  ...
  return {
    ...
    // datePublished/dateModified are valid on WebApplication (it inherits CreativeWork) and are a
    // documented AI-citation freshness signal — every programmatic/tool page sets these.
    ...(opts.datePublished ? { datePublished: opts.datePublished } : {}),
    dateModified: opts.dateModified ?? opts.datePublished ?? site.launched,
    publisher: { '@type': 'Organization', name: site.name, url: absoluteUrl('/') },
  };
}
...
/**
 * A page that is fundamentally a browsable table of the site's own pre-computed data (the
 * calculator/invest hubs). license/source describe how the numbers were produced, since this is
 * original computed output, not a copied third-party dataset (BUILDER_BRIEF honesty rule).
 */
export function datasetLd(opts: {
  ...
  return {
    ...
    isBasedOn: 'https://www.investor.gov/introduction-investing/investing-basics/compound-interest-calculator',
